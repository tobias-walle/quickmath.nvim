...
function M.run_command(opts)
  if opts.args ~= '' or opts.range == 0 then
    M.render_result_buffer(opts.args or '')
    return
  end
  ...
  for _, line in ipairs(lines) do
    ...
    table.sort(line_symbols, function(a, b)
  ...
  M.render_result_buffer(expression)
end

function M.render_result_buffer(expression)
  local bufnr = vim.api.nvim_create_buf(false, true)
  M.render_result_to_buffer(
    bufnr,
    expression,
    M.evaluate_expression(expression)
  )
  ...
  vim.api.nvim_create_autocmd({ 'TextChanged', 'TextChangedI' }, {
    ...
    callback = function()
      ...
      local new_expression = lines[1]:match('^([^=]+%s*)') or ''
      if new_expression then
        local result = M.evaluate_expression(new_expression)
        M.render_result_to_buffer(bufnr, new_expression, result)
      end
    end,
  })
...
function M.evaluate_expression(expression)
...
function M.render_result_to_buffer(bufnr, expression, result)
